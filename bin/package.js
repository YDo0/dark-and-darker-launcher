...
function build() {
    ...
    cp.execSync('npm ci -f', { stdio: 'inherit' });
...
function buildDarwin(cb) {
    ...
    electronPackager(Object.assign({}, all, darwin))
        .then(function (buildPath) {
            ...
            infoPlist.CFBundleDocumentTypes = [
                {
                    ...
                    CFBundleTypeIconFile:
                        path.basename(config.APP_FILE_ICON) + '.icns',
            ...
            cp.execSync(
                `cp ${config.APP_FILE_ICON + '.icns'} ${resourcesPath}`
            );
            ...
            function signApp(cb) {
                ...
                const signOpts = {
                    ...
                    platform: 'darwin',
                    identity:
                        'Developer ID Application: WebTorrent, LLC (5MAMC8G3L8)',
...
function buildWin32(cb) {
    ...
    electronPackager(Object.assign({}, all, win32))
        .then(function (buildPath) {
            ...
            if (process.platform === 'win32') {
                if (argv.sign) {
                    ...
                    const certificatePassword = fs.readFileSync(
                        path.join(CERT_PATH, 'authenticode.txt'),
                        'utf8'
                    );
...
function buildLinux(cb) {
    ...
    electronPackager(Object.assign({}, all, linux))
        .then(function (buildPath) {
    ...
    function packageDeb(filesPath, destArch, cb) {
        ...
        const options = {
            ...
            icon: {
                '48x48': path.join(
                    config.STATIC_PATH,
                    'linux/share/icons/hicolor/48x48/apps/webtorrent-desktop.png'
                ),
                '256x256': path.join(
                    config.STATIC_PATH,
                    'linux/share/icons/hicolor/256x256/apps/webtorrent-desktop.png'
                ),
            },
            ...
            mimeType: [
                ...
                'x-scheme-handler/stream-magnet',
            ],
            desktopTemplate: path.join(
                config.STATIC_PATH,
                'linux/webtorrent-desktop.ejs'
            ),
        };
