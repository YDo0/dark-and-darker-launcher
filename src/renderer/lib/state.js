...
function setupStateSaved() {
    ...
    const saved = {
        ...
        torrentsToResume: [],
        version:
            config.APP_VERSION /* make sure we can upgrade gracefully later */,
    };
    ...
    config.DEFAULT_TORRENTS.forEach((t, i) => {
        ...
        copyFileSync(
            ...
            path.join(
                config.POSTER_PATH,
                infoHash + path.extname(t.posterFileName)
            )
        );
        copyFileSync(
    ...
    function createTorrentObject(t) {
        ...
        return {
            ...
            displayName: t.name,
            posterFileName:
                parsedTorrent.infoHash + path.extname(t.posterFileName),
