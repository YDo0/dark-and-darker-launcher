...
function migrate_0_7_0(saved) {
    ...
    saved.torrents.forEach(function (ts) {
        ...
        if (ts.torrentPath) {
            if (
                path.isAbsolute(ts.torrentPath) ||
                ts.torrentPath.startsWith('..')
            ) {
                src = ts.torrentPath;
            } else {
...
function migrate_0_12_0(saved) {
    ...
    if (
        saved.prefs.openExternalPlayer == null &&
        saved.prefs.playInVlc != null
    ) {
    ...
    saved.torrents.forEach(function (torrentSummary) {
        if (!defaultTorrentFiles.includes(torrentSummary.torrentFileName))
            return;
