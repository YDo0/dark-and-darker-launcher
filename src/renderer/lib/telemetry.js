...
function send(state) {
    ...
    //             `Error sending telemetry, status code: ${res.statusCode}`
    //         );
    //     }
...
function logUncaughtError(procName, e) {
    ...
    if (e == null) {
        ...
        if (!e.error.message) {
            message =
                'Unexpected ErrorEvent.error: ' +
                Object.keys(e.error).join(' ');
        } else {
        ...
        if (!e.target) {
            ...
            message =
                'Unexpected resource loading error: ' + getElemString(e.target);
        } else {
            message =
                'Resource error ' +
                getElemString(e.target) +
                ': ' +
                e.target.error.code;
        }
    }
