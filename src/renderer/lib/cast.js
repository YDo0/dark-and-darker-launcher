...
function chromecastPlayer() {
        ...
        if (!selectedSubtitle) {
            ...
            ret.subServer = http
                .createServer((req, res) => {
                    ...
                    res.end(
                        Buffer.from(
                            selectedSubtitle.buffer.substr(21),
                            'base64'
                        )
                    );
                })
    ...
    function open() {
        const torrentSummary = state.saved.torrents.find(
            (x) => x.infoHash === state.playing.infoHash
        );
        serveSubtitles(function (subtitlesUrl) {
            ret.device.play(
                state.server.networkURL + '/' + state.playing.fileIndex,
                {
                    type: 'video/mp4',
...
function toggleMenu(location) {
    ...
    if (
        ...
        state.devices.castMenu.location === location
    ) {
