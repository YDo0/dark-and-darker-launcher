...
module.exports = class PlaybackController {
            ...
            state.location.go(
                {
                    ...
                    setup: (cb) => {
                        ...
                        if (index === undefined || initialized)
                            index = torrentSummary.mostRecentFileIndex;
                        if (index === undefined)
                            index = torrentSummary.files.findIndex(
                                TorrentPlayer.isPlayable
                            );
                        if (index === undefined)
                            return cb(new UnplayableTorrentError());
    ...
    updatePlayer(infoHash, index, resume, cb) {
        ...
        if (torrentSummary.status === 'paused') {
            ipcRenderer.once(
                'wt-ready-' + torrentSummary.infoHash,
                getAudioMetadata
            );
        } else {
            getAudioMetadata();
