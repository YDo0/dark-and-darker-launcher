...
module.exports = class TorrentController {
    ...
    torrentError(torrentKey, message) {
        ...
        if (torrentSummary) {
            console.log(
                'Pausing torrent %s due to error: %s',
                torrentSummary.infoHash,
                message
            );
    ...
    torrentMetadata(torrentKey, torrentInfo) {
        ...
        const hasDetailedFileInfo =
            torrentSummary.files && torrentSummary.files[0].path;
        ...
        if (!torrentSummary.torrentFileName)
        ...
        if (!torrentSummary.posterFileName)
            ipcRenderer.send('wt-generate-torrent-poster', torrentKey);
    }
    ...
    torrentProgress(progressInfo) {
        ...
        const progress =
            !progressInfo.hasActiveTorrents || progressInfo.progress === 1
                ? -1
        ...
        progressInfo.torrents.forEach((p) => {
            ...
            if (!torrentSummary) {
                console.log(
                    'warning: got progress for missing torrent %s',
                    p.torrentKey
                );
                return;
            }
    ...
    torrentFileModtimes(torrentKey, fileModtimes) {
        ...
        if (!torrentSummary)
            throw new Error(
                'Not saving modtimes for deleted torrent ' + torrentKey
            );
