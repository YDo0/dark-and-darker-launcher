...
function renderMedia(state) {
    ...
    if (mediaElement !== null) {
        ...
        if (
            ...
            isFinite(state.playing.setVolume)
        ) {
        ...
        if (state.playing.subtitles.selectedIndex !== -1) {
            ...
            file.selectedSubtitle =
                state.playing.subtitles.tracks[index].filePath;
        ...
        for (let j = 0; j < audioTracks.length; j++) {
            const isSelectedTrack =
                j === state.playing.audioTracks.selectedIndex;
    ...
    function onLoadedMetadata(e) {
        ...
        if (state.playing.type === 'video') {
            ...
            for (let i = 0; i < mediaElement.audioTracks.length; i++) {
                tracks.push({
                    label:
                        mediaElement.audioTracks[i].label || `Track ${i + 1}`,
...
function renderAudioMetadata(state) {
    ...
    if (common.label || common.catalognumber) {
        ...
        if (
            ...
            for (let n = 0; n < common.label.length; ++n) {
                releaseInfo.push(
                    common.label[0] + ' / ' + common.catalognumber[n]
                );
            }
        } else {
            if (common.label) {
    ...
    if (fileSummary.audioInfo.format.sampleRate) {
        format.push(
            Math.round(fileSummary.audioInfo.format.sampleRate / 100) / 10 +
                ' kHz'
        );
    }
...
function renderLoadingSpinner(state) {
    ...
    if (prog.files) {
        ...
        fileProgress = Math.floor(
            (100 * file.numPiecesPresent) / file.numPieces
        );
    }
...
function renderSubtitleOptions(state) {
    ...
    const noneClass =
        'radio_button_' + (noneSelected ? 'checked' : 'unchecked');
    return (
...
function renderPlayerControls(state) {
    ...
    const playbackCursorStyle = {
        left: 'calc(' + positionPercent + '% - 3px)',
    };
    const captionsClass =
