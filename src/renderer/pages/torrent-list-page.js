...
module.exports = class TorrentList extends React.Component {
    ...
    renderTorrent(torrentSummary) {
        ...
        const isSelected = true; //infoHash && state.selectedInfoHash === infoHash;
        ...
        return (
            <div
                ...
                // onClick={
                //     infoHash && dispatcher('toggleSelectTorrent', infoHash)
                // }
            >
    ...
    renderTorrentMetadata(torrentSummary) {
        const name = torrentSummary.name || 'Loading metadata...';
        const elements = [
    ...
    renderTorrentDetails(torrentSummary) {
        ...
        if (torrentSummary.error || !torrentSummary.files) {
            ...
            if (torrentSummary.error === 'path-missing') {
                ...
                message =
                    'Failed to load info. Click the download button to try again...';
            } else {
                // No file info, no infohash, trying to load from the DHT
                message = 'Downloading info...';
            }
            filesElement = (
            ...
            fileRows = fileRows.map((obj) =>
                this.renderFileRow(torrentSummary, obj.file, obj.index)
            );
    ...
    renderFileRow(torrentSummary, file, index) {
        ...
        if (
            ...
            progress =
                Math.floor(
                    (100 * fileProg.numPiecesPresent) / fileProg.numPieces
                ) + '%';
        }
        ...
        return (
            <tr key={index} onClick={handleClick}>
                ...
                >
                    {/* <i className='icon deselect-file'>
    ...
    renderRadialProgressBar(fraction, cssClass) {
        ...
        return (
            <div
                key='radial-progress'
                className={'radial-progress ' + cssClass}
            >
