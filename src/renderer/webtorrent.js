...
function init() {
    ...
    ipc.on('wt-set-download-limit', (e, speed) =>
        setDownloadSpeedLimit(speed)
    );
    ipc.on('wt-set-upload-limit', (e, speed) =>
...
function getTorrentProgress() {
    ...
    const torrentProg = client.torrents.map(function (torrent) {
        const fileProg =
            torrent.files &&
            torrent.files.map(function (file, index) {
                const numPieces = file._endPiece - file._startPiece + 1;
                let numPiecesPresent = 0;
                for (
                    let piece = file._startPiece;
                    piece <= file._endPiece;
                    piece++
                ) {
...
function selectFiles(torrentOrInfoHash, selections) {
    ...
    for (let i = 0; i < selections.length; i++) {
        ...
        if (selections[i]) {
            ...
            console.log(
                'deselecting file ' + i + ' of torrent ' + torrent.name
            );
            file.deselect();
        }
    }
...
function setDownloadSpeedLimit(speed) {
    console.log('Setting download speed limit (bytes/second): ' + speed);
    client.throttleDownload(speed);
}

function setUploadSpeedLimit(speed) {
    console.log('Setting upload speed limit (bytes/second): ' + speed);
